...
if (defined('IS_PHONE')) {
    ...
    $hyper_cache_is_mobile = preg_match('#()#i', $_SERVER['HTTP_USER_AGENT']);
}
...
$hc_file = realpath(__DIR__ . '/cache/hyper-cache/' . strtolower($_SERVER['HTTP_HOST']) . $hc_uri . '/index' . $hyper_cache_group . '.html');
...
if (0 && $hyper_cache_is_bot) {
    ...
    if (1 > 0 && $hc_file_time < time() - (1 * 3600)) {
...
if (0) {
    if (0 != 0) {
        $hc_cache_max_age = 0 * 3600;
    } else {
        // If there is not a default expire time, use 24 hours.
        if (1 > 0) {
            $hc_cache_max_age = time() + (1 * 3600) - $hc_file_time;
        } else {
